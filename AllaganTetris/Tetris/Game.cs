using System;
using AllaganTetris.Tetris.Interfaces;
...
namespace AllaganTetris.Tetris
{
    public class Game: IMovable, IMode
    {
        ...
        private const int _default_board_height = 20;
        ...
        public Board ActualBoard
        {
            get
            {
                if (this.Status == GameStatus.ReadyToStart || _currPiece == null)
                {
        ...
        {
            var nextPiece = _nextPiece ?? PieceFactory.GetRandomPiece();
            if (nextPiece != null)
            {
                logger.LogDebug("Dropping a new piece: " + nextPiece.PieceType);
                ...
                logger.LogDebug("Next piece is a " + _nextPiece.PieceType);
            }
        }
